...
public class GameState implements Cloneable{
    ...
    public int getOrientation(int player){
        return playerOrientation[player];
    }

    @Override
    public Object clone()throws CloneNotSupportedException{ //new method to clone a current gamestate to perform minimax on it. 
        GameState t = (GameState)super.clone();//clones state

        //makes copies of all of the arrays
        int length = this.occupied.length;
        boolean[][] occupiedCopy = new boolean[length][this.occupied[0].length];
        for(int i=0; i<this.occupied.length; i++){
            for(int j=0; j<this.occupied[i].length; j++){
                occupiedCopy[i][j]=this.occupied[i][j];
            }
        }
        t.occupied = occupiedCopy;
        List<Integer>[] copX = new List[this.playerX.length];
        List<Integer>[] copY = new List[this.playerY.length];
        for (int i = 0; i < copX.length; i++) {
            copX[i] = new ArrayList();
            copX[i].addAll(this.playerX[i]);
            copY[i] = new ArrayList();
            copY[i].addAll(this.playerY[i]);
        }
        t.playerX = copX;
        t.playerY = copY;
        int[] playerOrientationCopy = Arrays.copyOf(this.playerOrientation, this.playerOrientation.length);
        t.playerOrientation = playerOrientationCopy;
        int[] lastPlayerOrientationCopy = Arrays.copyOf(this.lastPlayerOrientation, this.lastPlayerOrientation.length);
        t.lastPlayerOrientation = lastPlayerOrientationCopy;
        boolean[] deadCopy = Arrays.copyOf(this.dead, this.dead.length);
        t.dead = deadCopy;
        int[] nrExtensionsLeftCopy = Arrays.copyOf(this.nrExtensionsLeft, this.nrExtensionsLeft.length);
        t.nrExtensionsLeft = nrExtensionsLeftCopy;
        
        //returns the clone of the state
        return t; 
    }

    public void printState(){ //function to help me bug fix!
        ...
        System.out.println(lastPlayerOrientation);
        System.out.println(NORTH);
        System.out.println(SOUTH);
        System.out.println(EAST);
        System.out.println(WEST);
        System.out.println(extensionsPerTarget);
        System.out.println(targetX);
        System.out.println(targetY);
        System.out.println(rand);
        System.out.println(dead);
        System.out.println(nrDead);
        System.out.println(nrPlayers);
        System.out.println(nrExtensionsLeft);
